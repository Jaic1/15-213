...
bool q_insert_head(queue_t *q, char *s)
{
    ...
    if(newh->value==NULL){free(newh);return false;}
    strcpy(newh->value,s);
...
bool q_insert_tail(queue_t *q, char *s)
{
	...
	if(newt->value==NULL){free(newt);return false;}
	strcpy(newt->value,s);
...
bool q_remove_head(queue_t *q, char *sp, size_t bufsize)
{
	...
	if(sp!=NULL){
		int l=strlen(q->head->value);
		if(l>bufsize-1)q->head->value[bufsize-1]=0;
		strcpy(sp,q->head->value);
	}
    /* You need to fix up this code. */
    free(q->head->value);
    if(q->head==q->tail)q->tail=NULL;
    list_ele_t *las=q->head;
    q->head = q->head->next;
    q->size--;
    free(las);
    return true;
...
void q_reverse(queue_t *q)
{
	...
	list_ele_t *cur=q->head,*las=NULL,*next=cur;
	q->head = q->tail;
	q->tail = cur;
	while(cur!=NULL){
		next=cur->next;
